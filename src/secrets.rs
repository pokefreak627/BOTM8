...
#[command]
async fn lakso(ctx: &Context, msg: &Message) -> CommandResult {
    msg.channel_id.send_message(&ctx.http, |c| {
        let mut embed = CreateEmbed::default();
        embed.title ("Congrats!:");    
        embed.description ("You found another hidden command. So far there are around 10 hidden commands, try and find them all just don't clog any channels apart from <#850056003642982490> \n \nThis is Lakso's 3D art place. \nHe does incredible 3D modeling which you can see showcased here: https://www.fiverr.com/s2/c1e8bd12c0 \n Wouldn't you know it he does commissions too. So get check it out and get yourself some nice models!");
        c.set_embed(embed.clone())
    }).await.unwrap();
    Ok(())
}
...
async fn psy(ctx: &Context, msg: &Message) -> CommandResult {
    msg.channel_id
        .send_message(&ctx.http, |c| {
            let mut embed = CreateEmbed::default();
            embed.title("Psy:");
            embed.description("some twat who makes a game idk");
            c.set_embed(embed.clone())
        })
        .await
        .unwrap();
